...
for i in range(m):
	if(len(sys.argv) == 2):
		...
		Kr.append(decrypt((key, N), int(input())))
...
for i in range(n):
	if(len(sys.argv) == 2):
		...
		Kc.append(decrypt((key, N), int(input())))

# print('Enter value of ITER_MAX')
ITER_MAX = int(input()) if len(sys.argv) == 2 else decrypt((key, N), int(input()))
...
path = sys.argv[1]
im.save('decrypted_images/' + path[path.rfind('\\')+1:])
print("Done!")
